...
public class FreshDownloadView extends View {
    ...
    //Which click on download process will start from here
    ...
    private class roundAnimListner implements AnimatorUpdateListener {
        public void onAnimationUpdate(ValueAnimator animation) {
            FreshDownloadView.this.mMarkArcAngle = (Float) animation.getAnimatedValue();
            FreshDownloadView.this.invalidate();
        }
    }
    ...
    private class firstAnimListner implements AnimatorUpdateListener {
        public void onAnimationUpdate(ValueAnimator animation) {
            FreshDownloadView.this.mMarkOkdegree = (Float) animation.getAnimatedValue();
            FreshDownloadView.this.invalidate();
        }
    }
    ...
    private class secondAnimListner implements AnimatorUpdateListener {
        public void onAnimationUpdate(ValueAnimator animation) {
            FreshDownloadView.this.mMarkOkstart = (Float) animation.getAnimatedValue();
            FreshDownloadView.this.invalidate();
        }
    }
    ...
    private void parseAttrs(TypedArray array) {
        if (array != null) {
            try {
                setRadius(array.getDimension(ACCESSIBILITY_LIVE_REGION_ASSERTIVE, getResources().getDimension(R.dimen.default_radius)));
                setCircularColor(array.getColor(ACCESSIBILITY_LIVE_REGION_NONE, getResources().getColor(R.color.default_circular_color)));
                setProgressColor(array.getColor(ACCESSIBILITY_LIVE_REGION_POLITE, getResources().getColor(R.color.default_circular_progress_color)));
                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
                    setCircularWidth(array.getDimension(AUTOFILL_TYPE_LIST, getResources().getDimension(R.dimen.default_circular_width)));
                    setProgressTextSize(array.getDimension(AUTOFILL_TYPE_DATE, getResources().getDimension(R.dimen.default_text_size)));
                }
            } finally {
                array.recycle();
            }
    ...
    private AnimatorSet getPrepareAnimator() {
        ...
        ValueAnimator downAnimaor = ValueAnimator.ofFloat(0.0f, 0.3f, 0.0f).setDuration(500);
        downAnimaor.setInterpolator(new DecelerateInterpolator());
        downAnimaor.addUpdateListener(new downloadAnimListner());
        ValueAnimator upAnimator = ValueAnimator.ofFloat(0.0f, 1.0f).setDuration(800);
        upAnimator.setInterpolator(new DecelerateInterpolator());
        upAnimator.addUpdateListener(new addUpdateListner());
        upAnimator.addListener(new upAnimAddListner());
        animatorSet.addListener(new setAnilAddListner());
        animatorSet.play(downAnimaor).before(upAnimator);
        animatorSet.addListener(new animSetsAddListner());
        return animatorSet;
    }

    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        super.onMeasure(widthMeasureSpec, heightMeasureSpec);
        int dy = ((getPaddingTop() + getPaddingBottom()) + getCurrentHeight());
    ...
    private AnimatorSet getDownloadOkAnimator() {
        AnimatorSet animatorSet = new AnimatorSet();
        ValueAnimator roundAnimator = ValueAnimator.ofFloat(0.0f, MARK_START_ANGLE).setDuration(100);
        roundAnimator.addUpdateListener(new roundAnimListner());
        roundAnimator.addListener(new roundAnimList());
        ValueAnimator firstAnimator = ValueAnimator.ofFloat(0.0f, 0.7f).setDuration(200);
        firstAnimator.setInterpolator(new AccelerateInterpolator());
        firstAnimator.addUpdateListener(new firstAnimListner());
        ValueAnimator secondAnimator = ValueAnimator.ofFloat(0.0f, 0.35f, 0.2f).setDuration(500);
        secondAnimator.addUpdateListener(new secondAnimListner());
